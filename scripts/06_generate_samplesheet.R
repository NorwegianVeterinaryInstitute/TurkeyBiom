...
filenames <- read_delim(
  "C:/Users/vi1511/temp/data/turkeybiom/filenames.txt",
  delim = "\t",
  col_names = FALSE
)
...
samplesheet <- filenames %>%
  mutate(sample = sub(".+/(.+_S.+_L00.)_R.+", "\\1", X1),
         run_accession = paste0("run", sub(".+_L00(.)_.+", "\\1", X1))) %>%
  mutate(sample = sub("(.+)_S.+", "\\1", sample),
  ...
  mutate(instrument_platform = "ILLUMINA",
         fasta = "") %>%
  select(sample, run_accession, instrument_platform, fastq_1, fastq_2, fasta) %>%
  mutate_at(vars(contains("fastq")),
            ~sub("\\.", "", .)) %>%
  mutate_at(vars(contains("fastq")),
            ~paste0("/cluster/work/users/hkaspersen/data", .)) %>%
